...
_solve_cvrptw_real = _mod.solve_cvrptw
...
def solve_cvrptw(instance, time_limit: float = 30.0) -> dict:
    """
    Chama o solve_cvrptw() do solver.py e devolve um dict compatível
    com o visualizador. No Solomon, distância ≈ tempo (escala 1:1),
    por isso usamos speed_kmh=60 (dist/60*60 = dist).
    """
    raw = _solve_cvrptw_real(instance, time_limit=time_limit,
                              verbose=False, speed_kmh=60.0)

    if raw["obj_value"] is None:
        return {"routes": [], "obj_value": None,
                "num_vehicles": 0, "runtime": raw["runtime"]}
    ...
    for r in raw["routes"]:
        routes.append({
            ...
            "route":    r["route"],
            "load":     r["load"],
            "distance": round(r["distance"] / 10, 2),  # unidades Solomon → km
        })

    return {
        "routes":       routes,
        "obj_value":    raw["obj_value"] / 10,
...
def build_plotly_data(instance, result: dict) -> dict:
    ...
    for r in result["routes"]:
        ...
        for i in range(len(route) - 1):
            ...
            traces.append({
                ...
                "x": [mx], "y": [my],
                "marker": {
                    "symbol": "arrow", "size": 10, "color": color,
    ...
    traces.append({
        ...
        "hovertext": ctext, "hoverinfo": "text", "showlegend": True,
    })

    traces.append({
        "type": "scatter", "mode": "markers+text",
        "x": [depot.x], "y": [depot.y], "name": "Depósito",
        "marker": {
            "size": 18, "color": "#1F4E79", "symbol": "star",
            "line": {"color": "white", "width": 2},
        },
        "text": ["D"], "textposition": "top center",
        "textfont": {"size": 10, "color": "white"},
        "hovertext": [
            f"<b>Depósito</b><br>Pos: ({depot.x}, {depot.y})<br>"
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .summary h3 {{ color: #1F4E79; font-size: 0.85rem; margin-bottom: 8px;
    text-transform: uppercase; letter-spacing: 0.5px; }}
  .summary-grid {{ display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }}
  .metric {{ text-align: center; }}
  .metric .val {{ font-size: 1.4rem; font-weight: 700; color: #1F4E79; }}
  .metric .lbl {{ font-size: 0.7rem; color: #666; }}
  .routes-title {{ font-size: 0.85rem; font-weight: 700; color: #1F4E79;
...
<header>
  <div>
    ...
    <span>OR-Tools · Laboratório de Projeto 2025/2026 · UAL</span>
  </div>
</header>
<div class="container">
  <div id="chart"></div>
  <div class="sidebar">
    <div class="summary">
      ...
      <div class="summary-grid">
        ...
        <div class="metric"><div class="val">{total_dist} km</div><div class="lbl">Distância</div></div>
...
def build_ref_box(instance, result):
    ref = getattr(_mod, "SOLOMON_REFERENCE", {}).get(instance.name.upper())
...
if __name__ == "__main__":
    ...
    result = solve_cvrptw(instance, time_limit=30.0)
